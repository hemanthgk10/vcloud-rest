...
module VCloudClient
  class Connection
    ...
    ##
    # Compose a vapp from vApp spec provided
    #
    # Params:
    # - spec: vApp Spec
    def compose_vapp_from_spec(spec)
      builder = Nokogiri::XML::Builder.new do |xml|
        xml.ComposeVAppParams(
            'xmlns' => 'http://www.vmware.com/vcloud/v1.5',
            'xmlns:ovf' => 'http://schemas.dmtf.org/ovf/envelope/1',
            'name' => spec[:name]) do
          xml.Description spec[:description] || 'Generated by ruby deployer'
          xml.InstantiationParams do
            xml.NetworkConfigSection do
              xml['ovf'].Info 'Configuration parameters for logical networks'
              spec[:network_config].each { |network_config|
                xml.NetworkConfig(
                  ...
                  xml.Configuration {
                    xml.IpScopes {
                      xml.IpScope {
                        ...
                        xml.IpRanges {
                          network_config[:ip_ranges].each { |range|
                            xml.IpRange {
                              xml.StartAddress range[:start_address]
                              xml.EndAddress range[:end_address]
                            }
                          }
                        }
                      }
                    }
                    xml.ParentNetwork("href" => "#{@api_url}/network/#{network_config[:parent_network]}") if network_config[:parent_network]
                    ...
                    xml.Features {
                      ...
                      if network_config[:enable_dhcp]
                        xml.DhcpService {
                          xml.IsEnabled(network_config[:enable_dhcp] || "false") 
                          xml.DefaultLeaseTime(network_config[:default_lease_time]  || "3600" )
                          xml.MaxLeaseTime(network_config[:max_lease_time] || "7200")
                          xml.IpRange {
                                xml.StartAddress(network_config[:dhcp_start_address]) 
                                xml.EndAddress(network_config[:dhcp_end_address])
                          }
                        }
                      end 
                    }
          ...
          spec[:hosts].each do |vm_name, values|
            xml.SourcedItem {
              xml.Source('href' => "#{@api_url}/vAppTemplate/vm-#{values[:template_id]}", "name" => vm_name)
              xml.InstantiationParams {
                xml.NetworkConnectionSection(
                    'xmlns:ovf' => 'http://schemas.dmtf.org/ovf/envelope/1',
                    'type' => 'application/vnd.vmware.vcloud.networkConnectionSection+xml',
                    'href' => "#{@api_url}/vAppTemplate/vm-#{values[:template_id]}/networkConnectionSection/") {
      ...
      params = {
          "method" => :post,
          "command" => "/vdc/#{spec[:vdc]}/action/composeVApp"
      }
